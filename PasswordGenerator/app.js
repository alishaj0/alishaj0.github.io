...
function generatePassword(upper, lower, number, symbol, length){
    ...
    // 2. Filter out unchecked types (.filter() method - where only arguments that pass the test will be applied (true arguments))
        ...
        const checkedBoxes = upper + lower + number + symbol;
            // checkedBoxes = true/false checked checkboxes
                // if upper and numbers selected (upper = true, numbers = true), checkedBoxes length will be 2
    ...
    // 3. Loop over the length (length user provided) and call the generator function for each checked type
        ...
        for (i = 0; i < length; i += checkedBoxes){
                // i = 0;
                // i < length; = whatever is passed in for the length value from <input type="number">
                // i + checkedBoxes : each time through the loop, we will add filter result to checkedBoxes array SAME AS i = i + checkedBoxes
            checkedArray.forEach(type => {
                const funcName = type[0];
                console.log(funcName);
                // forEach() method with anonymous arrow function with (type) as parameter 
                // taking in each item through the UPDATED/NEW checkedArray 
